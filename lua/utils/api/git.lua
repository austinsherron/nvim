...
function Git.diff_files(staged)
  if staged ~= true then
    ...
    return map(Git.status(), function(s)
      return s.path
    end)
  end
  ...
  return map(relative, function(r)
    return root .. '/' .. r
  end)
end
...
function Git.stash(msg)
  local msg_arg = ternary(String.not_nil_or_empty(msg), function()
    return ' -m ' .. msg
  end, '')
...
function Git.clone(repo, dir, options)
  options = Table.map_items(options or {}, {
    keys = function(k, v)
      return fmt('--%s=%s', k, v)
    end,
  })
