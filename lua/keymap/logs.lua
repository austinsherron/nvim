...
local function make_type_binding(type, viewmode)
  return {
    ...
    function() Logs.open({ type = type, viewmode = viewmode }) end,
...
local function make_type_bindings(type)
  return Stream.new(ViewMode:values())
    :map(function(v) return make_type_binding(type, v) end)
    :collect()
end
...
local function get_logger_types()
  return Array.sorted(
    Table.values(LoggerType.all()),
    { key = function(t) return t.i end }
  )
end
